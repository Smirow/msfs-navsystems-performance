...
class NavSystemTouch_ScrollElement {
    ...
    update() {
        ...
        if (this.isScrollLocked) {
            ...
            if (this.frameWithoutMovement > 3 && (this.elementContainer.scrollTop % this.elementSize) != 0) {
                if (this.toBottom) {
...
class NavSystemTouch_ActiveFPL extends NavSystemElement {
    ...
    onExit() {
        let info = this.gps.currFlightPlanManager.getWaypoint(this.getSelectedIndex());
        if (info) {
            this.gps.lastRelevantICAOType = info.type;
            this.gps.lastRelevantICAO = info.icao;
        }
    }
    onEvent(_event) {
    }
    ...
    activateLegTo() {
        SimVar.SetSimVarValue("C:fs9gps:FlightPlanActiveWaypoint", "number", this.selectedElement.index);
    }
    waypointInfo() {
    }
    updateMenu() {
