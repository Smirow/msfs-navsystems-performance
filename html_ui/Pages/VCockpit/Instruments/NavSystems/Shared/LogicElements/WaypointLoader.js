...
class FacilityLoader {
    ...
    static get Instance() {
        if (!FacilityLoader._Instance) {
            FacilityLoader._Instance = new FacilityLoader();
    ...
    registerListener() {
        ...
        RegisterViewListener("JS_LISTENER_FACILITY", () => {
            ...
            Coherent.on("SendAirport", (data) => {
                this.loadedFacilities.push(data);
    ...
    getAirportDataCB(icao, callback) {
        icao = icao.trim();
        let airport = this.loadedFacilities.find(f => { return f.icao.trim() === icao; });
        if (airport) {
        ...
        let checkDataLoaded = () => {
            ...
            if (airport) {
                ...
                attempts++;
                if (attempts > 5) {
                    console.warn("getAirportDataCB not found for icao '" + icao + "', expect the unexpected.");
                    let n = this.loadingFacilities.indexOf(icao);
                    if (n >= 0) {
                        this.loadingFacilities.splice(n, 1);
                    }
        ...
        return new Promise((resolve) => {
            ...
            let loadedAirportCallback = () => {
                ...
                if (airport) {
                    ...
                    attempts++;
                    if (attempts > 5) {
                        console.warn("getAirportData not found for icao '" + icao + "', expect the unexpected.");
                        let n = this.loadingFacilities.indexOf(icao);
                        if (n >= 0) {
                            this.loadingFacilities.splice(n, 1);
                        }
    ...
    getIntersectionDataCB(icao, callback) {
        ...
        let intersection = this.loadedFacilities.find(f => {
            return (f.icao.trim() === icao) && (f.routes != undefined);
        });
        if (intersection) {
            return callback(intersection);
        }
        if (icao[0] !== "W") {
            console.warn("Icao mismatch trying to load Intersection of invalid icao '" + icao + "'");
        }
        if (this.loadingFacilities.indexOf(icao) === -1) {
        ...
        let checkDataLoaded = () => {
            ...
            if (intersection) {
                ...
                attempts++;
                if (attempts > 1000) {
                    console.warn("getIntersectionDataCB not found for icao '" + icao + "', expect the unexpected.");
                    this.loadedFacilities.push({
                        icao: icao,
                        name: "UNKNOWN",
                        lat: 0,
                        lon: 0,
    ...
    getNdbDataCB(icao, callback) {
        icao = icao.trim();
        let ndb = this.loadedFacilities.find(f => { return f.icao.trim() === icao; });
        if (ndb) {
            return callback(ndb);
        }
        if (icao[0] !== "N") {
        ...
        let checkDataLoaded = () => {
            ...
            if (ndb) {
                ...
                attempts++;
                if (attempts > 1000) {
                    console.warn("getNdbDataCB not found for icao '" + icao + "', expect the unexpected.");
                    let n = this.loadingFacilities.indexOf(icao);
                    if (n >= 0) {
                        this.loadingFacilities.splice(n, 1);
                    }
    ...
    getVorDataCB(icao, callback) {
        icao = icao.trim();
        let vor = this.loadedFacilities.find(f => { return f.icao.trim() === icao; });
        if (vor) {
            return callback(vor);
        }
        if (icao[0] !== "V") {
        ...
        let checkDataLoaded = () => {
            ...
            if (vor) {
                ...
                attempts++;
                if (attempts > 1000) {
                    console.warn("getVorDataCB not found for icao '" + icao + "', expect the unexpected.");
                    let n = this.loadingFacilities.indexOf(icao);
                    if (n >= 0) {
                        this.loadingFacilities.splice(n, 1);
                    }
    ...
    async getAllAirways(intersection, maxLength = 100) {
        ...
        if (intersectionInfo instanceof IntersectionInfo) {
