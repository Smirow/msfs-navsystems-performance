...
class WayPoint {
    ...
    SetICAO(_ICAO, _endLoadCallback = null) {
        this.icao = _ICAO;
        this.infos.icao = _ICAO;
        this.UpdateInfos(_endLoadCallback);
    }
    SetIdent(_Ident) {
        if (_Ident != "") {
            this.ident = _Ident;
    ...
    SetFromIFacility(data, callback = EmptyCallback.Void) {
        ...
        if (this.type === "A") {
            this.infos = new AirportInfo();
            this.infos.SetFromIFacilityAirport(data);
            return callback();
        }
        else if (this.type === "W") {
            this.infos = new IntersectionInfo();
            this.infos.SetFromIFacilityIntersection(data, callback);
        }
        else if (this.type === "V") {
            this.infos = new VORInfo();
            this.infos.SetFromIFacilityVOR(data);
            return callback();
        }
        else if (this.type === "N") {
            this.infos = new NDBInfo();
            this.infos.SetFromIFacilityNDB(data);
            return callback();
        }
    }
    imageFileName() {
...
class AirportInfo extends WayPointInfo {
    ...
    UpdateInfos(_CallBack = null, _LoadApproaches = false) {
        this.loaded = false;
        FacilityLoader.Instance.getAirportDataCB(this.icao, (data) => {
            this.SetFromIFacilityAirport(data);
            this.loaded = true;
            if (_CallBack) {
                _CallBack();
            }
        ...
        if (data.approaches) {
            for (let i = 0; i < data.approaches.length; i++) {
                ...
                for (let i = 0; i < approachData.finalLegs.length; i++) {
                    let waypoint = new ApproachWayPoint();
