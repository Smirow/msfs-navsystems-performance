...
class GPS_DirectTo extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        var infos = this.icaoSearchField.getWaypoint() ? this.icaoSearchField.getWaypoint().infos : new WayPointInfo();
        if (infos && infos.icao != '') {
    ...
    onSearchEnd() {
        if (this.icaoSearchField.duplicates.length > 0) {
            this.gps.switchToPopUpPage(this.duplicateWaypoints, () => {
                ...
                this.icaoSearchField.getWaypoint().UpdateInfos();
...
class GPS_ActiveFPL extends MFD_ActiveFlightPlan_Element {
    ...
    FPLDeleteFlightPlan_CB() {
        for (var i = this.gps.currFlightPlan.wayPoints.length - 1; i >= 0; i--) {
            SimVar.SetSimVarValue("C:fs9gps:FlightPlanDeleteWaypoint", "number", 0);
        }
        this.gps.SwitchToInteractionState(0);
